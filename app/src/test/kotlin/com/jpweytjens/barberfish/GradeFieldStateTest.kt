...
/**
 * Pins the GradeReading -> displayed FieldState mapping, closing the loop between the scan reducer
 * (GradeReadingScanTest) and what the user sees. Value strings are matched separator-agnostically
 * so the test holds regardless of the JVM's default locale.
 */
class GradeFieldStateTest {
    ...
    @Test
    fun unavailable_showsSearching() {
    ...
    @Test
    fun stale_holdsLastValueInGrey() {
    ...
    @Test
    fun fresh_showsValueWithGradeColor() {
    ...
    @Test
    fun fresh_withColorModeNone_usesDefaultColor() {
        val s = state(GradeReading.Fresh(7.0f), GradeFieldConfig(ZoneColorMode.NONE))
        assertEquals(FieldColor.Default, s.color)
    }

    @Test
    fun fresh_integerPrecision_roundsAndKeepsPercent() {
        val cfg =
            GradeFieldConfig(ZoneColorMode.TEXT, ZoneDisplayMode.INTEGER, showPercentSign = true)
        val s = state(GradeReading.Fresh(7.4f), cfg)
        assertEquals("7%", s.primary)
    }

    @Test
    fun fresh_decimalPrecision_noPercentSign() {
        val cfg =
            GradeFieldConfig(ZoneColorMode.TEXT, ZoneDisplayMode.FLOAT, showPercentSign = false)
        val s = state(GradeReading.Fresh(7.0f), cfg)
        assertTrue("expected no % sign, got '${s.primary}'", s.primary.matches(Regex("7[.,]0")))
    }

    @Test
    fun stale_integerNoPercent_holdsRoundedValue() {
        val cfg =
            GradeFieldConfig(ZoneColorMode.TEXT, ZoneDisplayMode.INTEGER, showPercentSign = false)
    ...
    @Test
    fun fresh_integerHalfUp_roundsTieAwayFromZero() {
        val cfg =
            GradeFieldConfig(ZoneColorMode.TEXT, ZoneDisplayMode.INTEGER, showPercentSign = true)
    ...
    @Test
    fun fresh_decimalHalfUp_roundsTieUp() {
        val cfg =
            GradeFieldConfig(ZoneColorMode.TEXT, ZoneDisplayMode.FLOAT, showPercentSign = true)
