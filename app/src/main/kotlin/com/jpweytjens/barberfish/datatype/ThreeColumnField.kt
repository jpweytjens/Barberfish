...
class HUDField(private val karooSystem: KarooSystemService) :
    ...
    override fun liveFlow(context: Context) =
        combine(
            ...
            .flatMapLatest { (cfg, zones, profile) ->
                ...
                ) { s, h, p -> HudState(s, h, p, cfg.colorMode) }
            }
    ...
    private fun toSpeed(state: StreamState, profile: UserProfile): FieldValue {
        val raw = (state as? StreamState.Streaming)?.dataPoint?.values?.get(DataType.Field.SPEED)
    ...
    private fun toHr(state: StreamState, profile: UserProfile, zones: ZoneConfig): FieldValue {
        val raw = (state as? StreamState.Streaming)?.dataPoint?.values?.get(DataType.Field.HEART_RATE)
            ?: return FieldValue.unavailable("HR")
        return FieldValue(
            primary = raw.toInt().toString(),
            unit = "bpm",
            label = "HR",
            color = FieldColor.Zone(hrZone(raw, profile.heartRateZones), profile.heartRateZones.size.coerceAtLeast(1), zones.hrPalette, isHr = true),
            iconRes = R.drawable.ic_col_hr,
        )
    }

    private fun toPower(state: StreamState, stream: PowerSmoothingStream, profile: UserProfile, zones: ZoneConfig): FieldValue {
        val raw = (state as? StreamState.Streaming)?.dataPoint?.values?.get(stream.fieldId)
            ?: return FieldValue.unavailable("Power")
        return FieldValue(
            primary = raw.toInt().toString(),
            unit = "W",
            label = stream.label,
            color = FieldColor.Zone(powerZone(raw, profile.powerZones), profile.powerZones.size.coerceAtLeast(1), zones.powerPalette, isHr = false),
