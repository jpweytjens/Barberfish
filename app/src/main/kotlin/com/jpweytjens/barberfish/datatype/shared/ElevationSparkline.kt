...
/**
 * Decodes the Karoo elevation polyline into a list of (distanceM, elevationM) pairs. Returns empty
 * list for blank or invalid input. Returns whatever was decoded up to the truncation point if
 * [encoded] ends mid-varint.
 */
 ...
 *
 * Repeatedly removes the interior point whose triangle with its two live neighbours has the
 * smallest area, stopping when the smallest remaining area ≥ [minAreaM2]. Endpoints are always
 * preserved. Input is `(distanceM, elevationM)` pairs, so the triangle area is in m² and has a
 * direct physical meaning: roughly the smallest bump (width × height) the simplifier refuses to
 * erase.
 *
...
internal fun visvalingamWhyatt(
    ...
    if (minAreaM2 <= 0f) return points
    // Area-tolerance mode: drop points until the smallest effective area meets the floor.
    return vwSimplify(points) { smallestArea, _ -> smallestArea >= minAreaM2 }
}

/**
 * Visvalingam-Whyatt simplified to a fixed vertex budget: keeps the [targetCount] most significant
 * points (by effective area) and drops the rest. Unlike the area-tolerance variant this
 * self-normalises across route lengths, so a given level reads equally coarse on a short route or a
 * long one. Used by the whole-route overview, where absolute m² thresholds are invisible at
 * full-route zoom. Returns [points] unchanged when it already has at most [targetCount] points.
 */
internal fun visvalingamToCount(
    points: List<Pair<Float, Float>>,
    targetCount: Int,
): List<Pair<Float, Float>> {
    if (points.size <= targetCount) return points
    return vwSimplify(points) { _, aliveCount -> aliveCount <= targetCount }
}

/**
 * Shared Visvalingam-Whyatt core. Repeatedly removes the interior point whose triangle with its two
 * live neighbours has the smallest area (the canonical VW ranking), stopping when [stop] returns
 * true for the smallest remaining area and the current live-point count. Endpoints are always
 * preserved. Input is `(distanceM, elevationM)` pairs, so the triangle area is in m² and has a
 * direct physical meaning.
 */
private fun vwSimplify(
    points: List<Pair<Float, Float>>,
    stop: (smallestArea: Float, aliveCount: Int) -> Boolean,
): List<Pair<Float, Float>> {
    ...
    while (true) {
        ...
        if (stop(top.area, aliveCount)) break
...
/**
 * Renders a Tufte-style elevation sparkline strip. Returns a bitmap plus the current ratchet range;
 * the bitmap is null if [elevationPoints] is empty.
 *
...
internal fun renderElevationSparkline(
    ...
    val newDisplayedRange =
        if (elevRange > displayedRange) elevRange
    ...
    fun toY(e: Float) =
        (heightPx -
                (e - elevMin) / newDisplayedRange * (heightPx - 2 * MARKER_PAD_PX) -
                MARKER_PAD_PX)
    ...
    val pastEnd =
        visible.indexOfFirst { it.first > positionM }.let { if (it < 0) visible.size else it }
    val aheadStart =
        visible.indexOfFirst { it.first >= positionM }.let { if (it < 0) visible.size else it }
    ...
    if (aheadSilPts.isNotEmpty()) {
        ...
        paint.color =
            (if (isNightMode) SPARKLINE_SILHOUETTE_NIGHT else SPARKLINE_SILHOUETTE_DAY).toArgb()
        val path =
            Path().apply {
                moveTo(dotX, dotY)
    ...
    if (pastSilPts.isNotEmpty()) {
        paint.style = Paint.Style.FILL
        paint.color =
            (if (isNightMode) SPARKLINE_PAST_OVERLAY_NIGHT else SPARKLINE_PAST_OVERLAY_DAY).toArgb()
        val path =
            Path().apply {
    ...
    if (showPois && poiDistances.isNotEmpty()) {
        val aheadFill =
            (if (isNightMode) SPARKLINE_POI_FILL_NIGHT else SPARKLINE_POI_FILL_DAY).toArgb()
        val aheadStroke =
...
/**
 * Linear-interpolates the elevation at a given route distance from a sorted-by-distance list of
 * `(distanceM, elevationM)` polyline points. Returns null on an empty input. Distances outside the
 * polyline range clamp to the first/last sample.
 */
...
/**
 * Climb-only visibility for the HUD sparkline, shared by the live flow and the config preview. In
 * [SparklineMode.CLIMBS] each climb runs through three phases as you ride up to it: a counter phase
 * ("Climb n/total" text) one approach-length before the profile reveals, then the elevation profile
 * pinned to the climb (foot → top, left edge tracking you through the approach), then it hides past
 * the summit. [SparklineMode.OFF] hides it; [SparklineMode.ON] shows it with no window override.
 * Approach distance scales with the PCS climb score (see [climbApproachM]).
 */
        ...
        SparklineMode.CLIMBS -> {
            ...
            val active =
                frames
                    .withIndex()
                    .filter { (_, f) ->
                        positionM in (f.foot - 2f * f.approachM)..(f.summit + f.tailM)
                    }
            ...
            when {
                ...
                positionM < active.value.foot - active.value.approachM ->
                    ...
                    // Profile phase: left edge tracks the rider through the approach (lead-in
                    // shrinks),
                    // then locks at the foot; the lead margin keeps the dot off the left edge and
                    // the
...
private fun buildWarpedXMapper(
    ...
    return { routeDistanceM ->
        val windowFraction =
            ((routeDistanceM - windowStart) / (windowEnd - windowStart)).coerceIn(0f, 1f)
...
/**
 * Generates a synthetic elevation fixture: flat lead-in → climb → flat run-out. All fixtures span
 * 0–20 km with the climb starting at 10 km.
 *
...
private fun syntheticClimbFixture(gainM: Float, grade: Float): List<Pair<Float, Float>> {
    ...
    d = climbEnd + 50f
    while (d < routeEnd) {
        points.add(round1(d) to topElev)
        d += 50f
    }
...
/**
 * Generates a synthetic elevation fixture from a sequence of segments. Each segment is a (lengthM,
 * grade) pair. Positive grade = uphill, 0 = flat. Points sampled every 50m on flat, every 20m on
 * climbs.
 */
...
internal fun gain20WallFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(20f, 0.20f) // 100m @ 20%, L×G²=4.0

internal fun gain20SteepFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(20f, 0.10f) // 200m @ 10%, L×G²=2.0

internal fun gain20ModerateFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(20f, 0.05f) // 400m @  5%, L×G²=1.0

internal fun gain20GentleFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(20f, 0.03f) // 667m @  3%, L×G²=0.6

// 50m gain — varying difficulty (L×G²)
internal fun gain50WallFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(50f, 0.20f) // 250m @ 20%, L×G²=10.0

internal fun gain50SteepFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(50f, 0.10f) // 500m @ 10%, L×G²=5.0

internal fun gain50ModerateFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(50f, 0.05f) // 1km  @  5%, L×G²=2.5

internal fun gain50GentleFixture(): List<Pair<Float, Float>> =
    syntheticClimbFixture(50f, 0.03f) // 1.7km@  3%, L×G²=1.5

// Double ramp: 50m gain @ 10% → 2km flat → 50m gain @ 10%
internal fun doubleRampFixture(): List<Pair<Float, Float>> =
...
internal fun wallThenGentleFixture(): List<Pair<Float, Float>> =
    syntheticProfileFixture(
        listOf(
            5_000f to 0f, // 5km flat lead-in
...
internal val ELEVATION_FIXTURES: LinkedHashMap<String, () -> List<Pair<Float, Float>>> =
    linkedMapOf(
...
/**
 * Mock climb ranges for [rvvElevationFixture] — used in debug/preview to verify the blue climb
 * overlay without needing a live route. Both ranges are clearly uphill in the polyline so the
 * renderer's polyline-verified uphill check accepts them.
 */
internal fun rvvClimbsFixture(): List<Pair<Float, Float>> =
    listOf(
...
/**
 * Mock POI distances for [rvvElevationFixture] — used in debug/preview to verify POI marker
 * rendering without needing a live route. Includes a marker at the top of the Muur climb plus a
 * couple of others so we can see clustering and spacing behaviour.
 */
internal fun rvvPoisFixture(): List<Float> =
    listOf(
...
internal fun rvvElevationFixture(): List<Pair<Float, Float>> =
    listOf(
        97.1f to 13.0f,
        171.4f to 13.0f,
        213.6f to 14.0f,
...
/**
 * Opening climb of the Col de Rates (Costa Blanca), from a real GPX trace, used by the debug sweep
 * when the HUD sparkline is in Climbs mode. A ~3.8 km descent lead-in, the ~9.8 km / 649 m
 * sustained climb (avg ~6.6%, PCS ≈ 107 → large approach tier), then a ~3.8 km descent lead-out —
 * so the climb-only reveal both appears and disappears as the sweep rolls past. Downsampled to ~100
 * m spacing to match [rvvElevationFixture].
 */
internal fun colDeRatesElevationFixture(): List<Pair<Float, Float>> =
    listOf(
        0.0f to 350.0f,
        108.4f to 342.0f,
        201.0f to 335.0f,
