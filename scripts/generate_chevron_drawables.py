...
a `_GRADE_BANDS*` list, then writes one Android vector drawable per colour to
`app/src/main/res/drawable/ic_climber_chevron_<rrggbb>.xml`. Stale variants
that no longer correspond to a current palette colour are deleted.
...
FIELD_COLORS_KT = (
    _ROOT
    / "app/src/main/kotlin/com/jpweytjens/barberfish/datatype/shared/FieldColors.kt"
)
